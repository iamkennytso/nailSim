import { canvasHeight, canvasWidth, dialogBoxBackground, dialogBoxButtonBackground } from "../constants"
...
const gameOver = (game, win = false) => {
  ...
  const retryButton = game.add.rectangle((gameOverContainer.x - (gameOverContainer.width / 2)) + gameOverContainer.width / 4, gameOverContainer.y + (gameOverContainer.height / 6), 150, 50, dialogBoxButtonBackground, .75).setDepth(buttonTextureDepth).setOrigin(0.5)
  game.add.text(retryButton.x, retryButton.y, 'RETRY').setFontSize(20).setOrigin(0.5).setDepth(buttonTextDepth)
  const roleSelectButton = game.add.rectangle((gameOverContainer.x - (gameOverContainer.width / 2)) + (3 * (gameOverContainer.width / 4)), gameOverContainer.y + (gameOverContainer.height / 6), 150, 50, dialogBoxButtonBackground, .75).setDepth(buttonTextureDepth).setOrigin(0.5)
